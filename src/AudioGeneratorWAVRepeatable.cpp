/*
  ...
  AudioGeneratorWAVRepeatable
  Modified to enable seamless loop playback
...
AudioGeneratorWAVRepeatable::AudioGeneratorWAVRepeatable()
{
  running = false;
  file = NULL;
  output = NULL;
...
bool AudioGeneratorWAVRepeatable::stop()
{
...
bool AudioGeneratorWAVRepeatable::isRunning()
{
  return running;
}
...
bool AudioGeneratorWAVRepeatable::GetBufferedData(int bytes, void *dest)
{
...
bool AudioGeneratorWAVRepeatable::GetBufferedData(int bytes, void *dest)
{
  ...
  if (Repeat) { // ftsy
    return GetBufferedDataRepeatable(bytes, dest);
  }
...
/*
bool AudioGeneratorWAVRepeatable::GetBufferedDataRepeatable(int bytes, void *dest)
{
  uint8_t *p = reinterpret_cast<uint8_t*>(dest);
  ...
  while (bytes--) {
    ...
    if (buffPtr >= buffLen) {
      ...
      while (buffLen < buffSize) {
        uint16_t sz = file->read(&buff[buffLen], 1);
        if (sz < 1) {
          file->seek(-1*dataLen, SEEK_CUR);
          continue;
        }
        buffLen ++;
      }
...
bool AudioGeneratorWAVRepeatable::GetBufferedDataRepeatable(int bytes, void *dest)
{
  ...
  while (bytes--) {
    ...
    if (buffPtr >= buffLen) {
      ...
      if (buffLen < buffSize) { // End of file, seek to beginning of the data
        //file->seek(-1*dataLen, SEEK_CUR); // noise!
        file->seek(dataPos, SEEK_SET);
        buffLen += file->read(&buff[buffLen], buffSize - buffLen);
      }
    }
    /*
    if (buffPtr >= buffLen)
...
bool AudioGeneratorWAVRepeatable::loop()
{
...
bool AudioGeneratorWAVRepeatable::ReadWAVInfo()
{
  uint32_t u32;
  uint16_t u16;
  int toSkip;
  ...
  // Header == "RIFF"
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (01)\n"));
    return false;
  };
  if (u32 != 0x46464952) {
  ...
  // Skip ChunkSize
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (02)\n"));
    return false;
  };
  dataPos += 4;
  ...
  // Format == "WAVE"
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (03)\n"));
    return false;
  };
  if (u32 != 0x45564157) {
  ...
  while (1) {
    if (!ReadU32(&u32)) {
      Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (04)\n"));
      return false;
    };
  ...
  // subchunk size
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (05)\n"));
    return false;
  };
  if (u32 == 16) { toSkip = 0; }
  ...
  // AudioFormat
  if (!ReadU16(&u16)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (06)\n"));
    return false;
  };
  if (u16 != 1) {
  ...
  // NumChannels
  if (!ReadU16(&channels)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (07)\n"));
    return false;
  };
  ...
  audioLogger->printf("channels=%d\n", channels);

  // SampleRate
  if (!ReadU32(&sampleRate)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (08)\n"));
    return false;
  };
  if (sampleRate < 1) {
  ...
  audioLogger->printf("sampleRate=%d\n", sampleRate);

  // Ignore byterate and blockalign
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (09)\n"));
    return false;
  };
  dataPos += 4;
  if (!ReadU16(&u16)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (10)\n"));
    return false;
  };
  dataPos += 2;
  ...
  if (!ReadU16(&bitsPerSample)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (11)\n"));
    return false;
  };
  ...
  audioLogger->printf("bitsPerSample=%d\n", bitsPerSample);

  // Skip any extra header
  while (toSkip) {
    uint8_t ign;
    if (!ReadU8(&ign)) {
      Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (12)\n"));
      return false;
    };
    toSkip--;
    dataPos += 1;
  ...
  do {
    // id == "data"
    if (!ReadU32(&u32)) {
      Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (13)\n"));
      return false;
    };
    dataPos += 4;
    ...
    if (!ReadU32(&u32)) {
      Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (14)\n"));
      return false;
    };
    dataPos += 4;
  ...
  if (!ReadU32(&u32)) {
    Serial.printf_P(PSTR("AudioGeneratorWAV::ReadWAVInfo: failed to read WAV data (15)\n"));
    return false;
  };
  availBytes = u32;
  dataLen = u32;
  ...
  audioLogger->printf("dataLen=%d\n", dataLen);
  audioLogger->printf("dataPos=%d\n", dataPos);
...
bool AudioGeneratorWAVRepeatable::begin(AudioFileSource *source, AudioOutput *output)
{
  if (!source) {
